...
export const UserSchema = new Schema<IUserDoc>(
  {
    ...
    username: {
      ...
      minlength: [
        ...
        `Username must be at least ${USER_CONSTANTS.USERNAME.MIN_LENGTH} characters long`,
      ],
      maxlength: [
        USER_CONSTANTS.USERNAME.MAX_LENGTH,
        `Username cannot exceed ${USER_CONSTANTS.USERNAME.MAX_LENGTH} characters`,
      ],
      trim: true,
      lowercase: true,
      match: [
      ...
      validate: {
        validator: function (this: any, username: string) {
          ...
          return !USER_CONSTANTS.USERNAME.RESERVED.includes(
            username.toLowerCase() as any,
          );
        },
    ...
    firstName: {
      ...
      trim: true,
      maxlength: [
        USER_CONSTANTS.LIMITS.NAME_MAX_LENGTH,
        `First name cannot exceed ${USER_CONSTANTS.LIMITS.NAME_MAX_LENGTH} characters`,
      ],
      match: [
        /^[a-zA-Z\s'-]+$/,
    ...
    lastName: {
      ...
      maxlength: [
        USER_CONSTANTS.LIMITS.NAME_MAX_LENGTH,
        `Last name cannot exceed ${USER_CONSTANTS.LIMITS.NAME_MAX_LENGTH} characters`,
      ],
      match: [
        /^[a-zA-Z\s'-]+$/,
    ...
    bio: {
      type: String,
      maxlength: [
        USER_CONSTANTS.LIMITS.BIO_MAX_LENGTH,
        `Bio cannot exceed ${USER_CONSTANTS.LIMITS.BIO_MAX_LENGTH} characters`,
      ],
      trim: true,
    },
...
UserSchema.index({ email: 1 }, { sparse: true } as any);
...
UserSchema.index({ resetPasswordExpires: 1 }, { expireAfterSeconds: 0 } as any);
UserSchema.index(
  { emailVerificationExpires: 1 },
  { expireAfterSeconds: 0 } as any,
);
UserSchema.index(
  { phoneVerificationExpires: 1 },
  { expireAfterSeconds: 0 } as any,
);
UserSchema.index({ lockoutUntil: 1 }, { expireAfterSeconds: 0 } as any);
